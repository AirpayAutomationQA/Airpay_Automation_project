...
public class AirPay_CAPanel_BusinessLogic extends Airpay_CAPanel_PageObject {
	...
	public void CA_Panel_Login() throws Exception{
		try{
			...
			String MA_URL = Excel_Handling.Get_Data(TC_ID, "PaymentPage_URL").trim();
			...
			if(Assert.isElementDisplayed(driver, CAloginID, "USER ID")){
				Assert.inputText(driver, CAloginID, Excel_Handling.Get_Data(TC_ID, "MA_UserID").trim(), "CA Panel USer ID");
				Assert.inputText(driver, CAPwd, Excel_Handling.Get_Data(TC_ID, "MA_Password").trim(), "Ca panel PWD");
